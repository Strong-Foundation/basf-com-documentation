...
func main() {
	...
	var endLoop int = 100                                  // Number of pages to fetch (only page 0 in this case)
	for index := 0; index <= endLoop; index++ {            // Loop through each page index
		url := fmt.Sprintf("https://dss.wcms.basf.com/v1/results?locale=en-US&limit=2000&page=%d", index) // API URL with pagination
